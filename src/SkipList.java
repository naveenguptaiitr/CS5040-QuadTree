...
public class SkipList<K extends Comparable<? super K>, V>
     ...
     * 
     * @return ArrayList foundElement
     *         contains the KVPairs for the specified key
     */
    public ArrayList<KVPair<K, V>> search(K key) {
        SkipNode x = head; // Dummy header node
        ArrayList<KVPair<K, V>> foundElement = new ArrayList<KVPair<K, V>>();
    ...
    public KVPair<K, V> remove(K key) {
        ...
        x = x.forward[0];
        if ((x != null) && (x.element().getKey().compareTo(key) == 0)) {
            int length = x.forward.length;
            SkipNode[] tempVar = new SkipList.SkipNode[length];
            for (int j = 0; j < length; j++) {
                tempVar[j] = x.forward[j];
            }
           // removing the element and updating the pointers
            for (int k = 0; k < length; k++) {
                update[k].forward[k] = tempVar[k];
            }
            size--;
            return x.element();
        } // Got it
    ...
    public void dump() {
        ...
        while (x != null) {
            ...
            if (x.element() != null) {
                ...
                KVPair<K, V> rectObject = x.element();

                QuadPoint quadPoint = (QuadPoint)rectObject.getValue();

                String rectDetails = String.format(
                    ("(%1$s, %2$s, %3$s)"), rectObject.getKey(),
                    quadPoint.getPointObject().x, quadPoint.getPointObject().y);

                System.out.println("level: " + x.forward.length
                    + " Value: " + rectDetails);
            }
            else {

                System.out.println("level: " + x.forward.length
                    + " Value: " + String.format("%s", x.element()));
            }
